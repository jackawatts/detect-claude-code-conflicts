...
- **Apostrophes in prose look like string delimiters.** "don't" opened a spurious single-quoted span, which returned `# Corrections` at 255 characters instead of 1245. Fixed by requiring the span to contain the complete anchor.
...
## Settled by the extraction

- The harness reminder slot resolves as `${IFm(t,"lean")}`, so the argument is literally `"lean"`.
- The reversibility paragraph has an alternate ending behind `${VWd(e)?"":...}`: "If what you find contradicts how it was described, or you didn't create it, surface that instead of proceeding".
- The opening line has a fallback: `${e!==null? ... :"You are an interactive agent that helps users with software engineering tasks."}`.
- Three sibling identity strings exist, selected by launch context — plain CLI, CLI within the Agent SDK, and a bare Agent SDK agent.
